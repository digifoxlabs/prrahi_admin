...
function posOrder(){
    return {
        ...
        distributorOrderSettings: {
            checkStockBeforeOrder: @js(\App\Models\Setting::get('distributor_orders', 'check_stock_before_order', '0') == '1'),
            allowZeroStockOrder: @js(\App\Models\Setting::get('distributor_orders', 'allow_zero_stock_order', '0') == '1'),
        },
        previewUrl: @json(route($routePrefix.'.orders.preview')),
        csrfToken: @json(csrf_token()),

        selectedDistributorId: {{ $order->distributor_id }},
        selectedDistributorTown: @json($order->town ?? ''),
        selectedDistributorDistrict: @json($order->district ?? ''),
        ...
        isStockCheckEnabled(){
            return Boolean(this.distributorOrderSettings?.checkStockBeforeOrder);
        },

        canOrderZeroStock(){
            return Boolean(this.distributorOrderSettings?.allowZeroStockOrder);
        },
