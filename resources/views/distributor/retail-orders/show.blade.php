...
<div class="mx-auto max-w-(--breakpoint-2xl) p-4 md:p-6">
    ...
    <div x-data="{ showModal: false, action: '', showInvoiceRemove: false, showInvoiceModal: false, showAssignDistributor: false }">
    ...
    {{-- Assign Distributor --}}
    <div
    x-show="showAssignDistributor"
    x-cloak
    class="fixed inset-0 z-50 flex items-center justify-center">

    <!-- Backdrop -->
    <div class="absolute inset-0 bg-black/50"
         @click="showAssignDistributor=false"></div>

    <!-- Modal -->
    <div class="bg-white rounded-xl p-6 w-full max-w-md z-10">
        <h3 class="text-lg font-semibold mb-4">
            Assign Distributor
        </h3>

        @if(!$eligibleDistributors)
            <p class="text-sm text-red-600">
                No distributor assigned to this retailer.
            </p>
        @else
            <form method="POST"
                ...
                <div class="mb-4">
                    <label class="text-sm font-medium">Select Distributor</label>
                ...
                <div class="flex justify-end gap-3">
                    <button type="button"
                            @click="showAssignDistributor=false"
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        ...
        <!-- Distributor -->
        <div>
            <h3 class="font-semibold mb-1">Retailer</h3>
            <p class="text-sm">{{ $order->retailer->retailer_name }}</p>
            <p class="text-xs text-gray-600">
                {{ $order->retailer->contact_person }}
                {{ $order->retailer->contact_number }}
            </p>
        </div>
        ...
        <div>
            ...
            <pre class="text-sm text-gray-700 whitespace-pre-wrap bg-gray-50 border rounded p-3">{{ $order->billing_address }}</pre>
        </div>
...
<div class="flex justify-start gap-3 mb-4">
        ...
        @if($order->status === 'pending')   
            ...
            <button
                @click="showAssignDistributor=true"
                class="px-4 py-2 bg-blue-600 text-white rounded-lg">
                Assign Distributor
            </button>
