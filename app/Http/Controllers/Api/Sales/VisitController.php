...
class VisitController extends Controller
{
    ...
    //     $distributors = Distributor::where('sales_persons_id', $salesPersonId)
    //         ->leftJoin('visit_notes', function ($join) use ($salesPersonId) {
    //             $join->on('visit_notes.entity_id', '=', 'distributors.id')
    //                  ->where('visit_notes.entity_type', 'distributor')
    //                  ->where('visit_notes.sales_person_id', $salesPersonId);
    //         })
    //         ->selectRaw("
    //             distributors.id as entity_id,
    //             distributors.firm_name as entity_name,
    //             'distributor' as entity_type,
    //             MAX(visit_notes.message) as last_message,
    //             MAX(visit_notes.created_at) as last_visited_at
    //         ")
    //         ->groupBy('distributors.id', 'distributors.firm_name');

    //     $retailers = Retailer::where('sales_persons_id', $salesPersonId)
    //         ->leftJoin('visit_notes', function ($join) use ($salesPersonId) {
    //             $join->on('visit_notes.entity_id', '=', 'retailers.id')
    //                  ->where('visit_notes.entity_type', 'retailer')
    //                  ->where('visit_notes.sales_person_id', $salesPersonId);
    //         })
    //         ->selectRaw("
    //             retailers.id as entity_id,
    //             retailers.name as entity_name,
    ...
    //         ->where('distributors.sales_persons_id', $salesPersonId)
    //         ->leftJoin('visit_notes', function ($join) use ($salesPersonId) {
    //             $join->on('visit_notes.entity_id', '=', 'distributors.id')
    //                 ->where('visit_notes.entity_type', 'distributor')
    //                 ->where('visit_notes.sales_person_id', $salesPersonId);
    ...
    //             SELECT vn.message
    //             FROM visit_notes vn
    //             WHERE vn.entity_id = distributors.id
    //               AND vn.entity_type = "distributor"
    //               AND vn.sales_person_id = ' . $salesPersonId . '
    //             ORDER BY vn.created_at DESC
    //             LIMIT 1
    //         ) as last_message'),
    //             DB::raw('(
    //             SELECT vn.created_at
    //             FROM visit_notes vn
    //             WHERE vn.entity_id = distributors.id
    //               AND vn.entity_type = "distributor"
    //               AND vn.sales_person_id = ' . $salesPersonId . '
    ...
    //         ->where('retailers.appointed_by_id', $salesPersonId)
    //         ->leftJoin('visit_notes', function ($join) use ($salesPersonId) {
    //             $join->on('visit_notes.entity_id', '=', 'retailers.id')
    //                 ->where('visit_notes.entity_type', 'retailer')
    //                 ->where('visit_notes.sales_person_id', $salesPersonId);
    ...
    //         ->select(
    //             'retailers.id as entity_id',
    //             'retailers.retailer_name as entity_name',
    //             DB::raw("'retailer' as entity_type"),
    //             DB::raw('(
    //             SELECT vn.message
    //             FROM visit_notes vn
    //             WHERE vn.entity_id = retailers.id
    //               AND vn.entity_type = "retailer"
    //               AND vn.sales_person_id = ' . $salesPersonId . '
    //             ORDER BY vn.created_at DESC
    //             LIMIT 1
    //         ) as last_message'),
